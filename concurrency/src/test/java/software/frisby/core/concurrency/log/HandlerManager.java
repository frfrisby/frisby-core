...
/**
 * Manages the singleton {@link CapturingHandler} that is installed on the root JUL logger and
 * routes captured {@link SystemLogEvent} instances to the currently active
 * {@link DefaultSystemLogVerifier}.
 * <p>
 * The handler and the root logger level are configured once when this class is first loaded.
...
final class HandlerManager {
    // Never read after initialization.  The field exists solely to trigger install() exactly
    // once via static field initialization when this class is first loaded.  install() registers
    // the CapturingHandler on the root JUL logger as a permanent side effect; the root logger
    // then holds the only strong reference needed to keep the handler alive.
